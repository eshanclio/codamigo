...
func TestNew(t *testing.T) {
	...
	idx := indexer.New(nil, emb, s, w, 1, 0, nil, nil)
	if idx == nil {
...
func TestIndexFiles_DeletesMissing(t *testing.T) {
	...
	if err := s.SetFileHash(ctx, "/nonexistent/file.go", "h1"); err != nil {
...
func TestIndexFiles_EmbeddingFailurePreservesOldChunks(t *testing.T) {
	...
	idx := indexer.New(c, goodEmb, s, w, 1, 0, nil, nil)
	...
	oldHash, err := s.FileHash(ctx, goFile)
	if err != nil {
	...
	idx2 := indexer.New(c, badEmb, s, w, 1, 0, nil, nil)
	_ = idx2.IndexFiles(ctx, []string{goFile})

	// Verify old chunks are still intact (embedding failed before ReplaceByFile).
	hash, err := s.FileHash(ctx, goFile)
	if err != nil {
...
func TestIndex_ConcurrentProcessing(t *testing.T) {
	...
	emb := &fakeEmbedder{dim: dim}
	idx := indexer.New(nil, emb, s, w, 5, 0, nil, nil)
...
func TestIndexer_Progress(t *testing.T) {
	...
	t.Run("reports processed on new file", func(t *testing.T) {
		...
		idx := indexer.New(c, &fakeEmbedder{dim: dim}, s, w, 1, 0, nil, spy)
		if err := idx.Index(ctx); err != nil {
			t.Fatal(err)
		}
	...
	t.Run("reports skipped on hash match", func(t *testing.T) {
		...
		if err := s.SetFileHash(ctx, goFile, store.ContentHash(content)); err != nil {
			t.Fatal(err)
		}
		cfg := &config.Config{ProjectRoot: root}
		w, err := walker.New(root, cfg)
		...
		defer w.Close() //nolint:errcheck
		idx := indexer.New(nil, &fakeEmbedder{dim: dim}, s, w, 1, 0, nil, spy)
		if err := idx.Index(ctx); err != nil {
			t.Fatal(err)
		}
	...
	t.Run("reports skipped on oversized file", func(t *testing.T) {
		...
		defer w.Close() //nolint:errcheck
		idx := indexer.New(nil, &fakeEmbedder{dim: dim}, s, w, 1, 10, nil, spy)
		if err := idx.Index(ctx); err != nil {
			t.Fatal(err)
		}
		if got := spy.Skipped(); len(got) != 1 {
	...
	t.Run("nil progress does not panic", func(t *testing.T) {
		...
		defer w.Close() //nolint:errcheck
		idx := indexer.New(nil, &fakeEmbedder{dim: dim}, s, w, 1, 0, nil, nil)
		if err := idx.Index(ctx); err != nil {
			t.Fatal(err)
		}
...
func TestIndexer_Progress_Concurrent(t *testing.T) {
	...
	defer w.Close() //nolint:errcheck
	idx := indexer.New(nil, &fakeEmbedder{dim: dim}, s, w, 8, 0, nil, spy)
